...
# run-eval-claude.sh — Run a single eval using Claude Code (claude -p).
# Usage:
#   ./run-eval-claude.sh evals/01-fix-bug-off-by-one
#   ./run-eval-claude.sh evals/01-fix-bug-off-by-one --model sonnet
#   ./run-eval-claude.sh evals/01-fix-bug-off-by-one --model opus

usage() {
  echo "Usage: $0 <eval-dir> [--model <model>]"
  echo ""
  echo "Models: sonnet (default), opus, haiku, or full name like claude-sonnet-4-6"
  exit 1
}
...
# --- Step 4: Run Claude Code on the prompt ---
...
# --- Step 5: Restore EVAL.ts and run vitest ---
echo "[5/6] Grading with vitest..."
...
# --- Save JSON result ---
RESULT_FILE="$RESULTS_DIR/${EVAL_NAME}_claude_${TIMESTAMP}.json"
